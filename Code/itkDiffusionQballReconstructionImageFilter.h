 ...
 * \brief This class takes as input one or more reference images (acquired 
 * in the  absence of diffusion sensitizing gradients) and 'n' diffusion
 * weighted images and their gradient directions and computes an image of 
 * orientation distribution functions (ODFs).
 *
 * \par Inputs and Usage
 * There are two ways to use this class. When you have one reference image 
 * and \c n gradient images, you would use the class as
 * \code
 ...
 * images. This is convenient when the DWI images are read in using the 
 * <a href="http://wiki.na-mic.org/Wiki/index.php/NAMIC_Wiki:DTI:Nrrd_format">
 * NRRD</a>  format. Like the Nrrd format, the reference images are those 
 * components of the  vectorImage whose gradient direction is (0,0,0). If more 
 * than one reference image is present, they are averaged prior to the 
 * reconstruction.
 *
 * \par Outputs
 ...
 * than this.
 * \li BValue - B-value for normalization
 ...
 * \li NrEquatorSamplingPoints - number of sampling points on equator when
 * performing Funk Radeon Transform (FRT)
 * \li NormalizationMethod - Standard or no normalization for now
 * 
 * \par Template parameters
 * The class is templated over 
 * \li the pixel type of the reference and gradient images 
 * (expected to be scalar data types) 
 * \li the internal pixel type of the ODFs.
 * \li the number of resulting Odf directions
 * \li the number of basis function centers for the sRBF (spherical radial basis 
 * functions interpolation). If not set, they will default to NrOdfDirections.
 *  
 * \par References:
 * \li<a href=
 * "http://www3.interscience.wiley.com/cgi-bin/fulltext/109800293/PDFSTART">
 * [1]</a><em>Tuch DS, "Q-ball imaging", 
 * Magn Reson Med. 2004 Dec;52(6):1358-72.</em>
 * 
 */
...
class ITK_EXPORT DiffusionQballReconstructionImageFilter :
...
public:
  ...
  */
  ...
  typedef DiffusionQballReconstructionImageFilter 
                                      Self;
  ...
  typedef Image< GradientPixelType, 3 >
                                      GradientImageType;
  ...
  void AddGradientImage( 
    const GradientDirectionType &, const GradientImageType *image);
  ...
  void SetGradientImage( 
    GradientDirectionContainerType *, const GradientImagesType *image);
  ...
  { 
    return ( static_cast< ReferenceImageType *>(
  ...
  /** Number of points sampled on the equator when applying FRT */
  itkSetMacro( NumberOfEquatorSamplingPoints, unsigned int );
  itkGetMacro( NumberOfEquatorSamplingPoints, unsigned int );
...
#ifdef __QBR_TIMING__
  static itk::TimeProbesCollectorBase m_Collector;
#endif //__QBR_TIMING__
...
private:
  ...
  /* Reconstruction matrix is multiplied to signal vector for reconstruction */

...
#define QBALL_RECON_PI       3.14159265358979323846
  ...
  class TOdfPixelType,
    ...
    itk::TimeProbesCollectorBase itk::DiffusionQballReconstructionImageFilter< 
    TReferenceImagePixelType, TGradientImagePixelType, TOdfPixelType, 
    NrOdfDirections, NrBasisFunctionCenters>::m_Collector;
  ...
  m_NumberOfBaselineImages(1),
    ...
    m_NormalizationMethod(QBR_STANDARD)
  {
    ...
    ::BeforeThreadedGenerateData()
  {
    ...
    // Otherwise we must have a container of (numberOfInputs-1) itk::Image. 
    // Check to make sure
    if ( numberOfInputs == 1
...
#ifdef __QBR_TIMING__
  m_Collector.Start( "ComputeReconMatrix" );
#endif //__QBR_TIMING__
    this->ComputeReconstructionMatrix();
#ifdef __QBR_TIMING__
  m_Collector.Stop( "ComputeReconMatrix" );
#endif //__QBR_TIMING__
    ...
    {
      typename ReferenceImageType::Pointer img = static_cast< 
        ReferenceImageType * >(this->ProcessObject::GetInput(0));
      m_BZeroImage->SetSpacing( img->GetSpacing() );    
      m_BZeroImage->SetOrigin( img->GetOrigin() );      
      m_BZeroImage->SetDirection( img->GetDirection() );
    ...
    {
      typename GradientImagesType::Pointer img = static_cast< 
        GradientImagesType * >( 
...
#ifdef __QBR_TIMING__
  m_Collector.Start( "Reconstruction" );
#endif //__QBR_TIMING__
  ...
  class TOdfPixelType,
    int NrOdfDirections,
    int NrBasisFunctionCenters>
    typename itk::DiffusionQballReconstructionImageFilter<
      TReferenceImagePixelType, TGradientImagePixelType, TOdfPixelType, 
      NrOdfDirections, NrBasisFunctionCenters>::OdfPixelType 
    itk::DiffusionQballReconstructionImageFilter<
      TReferenceImagePixelType, TGradientImagePixelType, TOdfPixelType, 
      NrOdfDirections, NrBasisFunctionCenters>::Normalize( 
      OdfPixelType odf, 
  ...
  class TOdfPixelType,
    int NrOdfDirections,
    int NrBasisFunctionCenters >
    vnl_vector<TOdfPixelType> itk::DiffusionQballReconstructionImageFilter<
      TReferenceImagePixelType, TGradientImagePixelType, TOdfPixelType, 
      NrOdfDirections, NrBasisFunctionCenters>
      ::PreNormalize( vnl_vector<TOdfPixelType> vec )
  {
    ...
    case QBR_STANDARD:
      ...
      // no normalization before reconstruction
    case QBR_NONE:
      {
        return vec;
        break;
    ...
    int ) 
  {
    ...
    // init output and b-zero image iterators
    typename OutputImageType::Pointer outputImage = 
      static_cast< OutputImageType * >(this->ProcessObject::GetOutput(0));
    ImageRegionIterator< typename Self::OutputImageType > oit(
      outputImage, outputRegionForThread);
    oit.GoToBegin();
    ImageRegionIterator< typename Self::BZeroImageType > oit2(
      m_BZeroImage, outputRegionForThread);
    oit2.GoToBegin();
    ...
    if( m_GradientImageTypeEnumeration == GradientIsInManyImages )
    {
      ...
      ImageRegionConstIterator< ReferenceImageType > 
        it(static_cast< ReferenceImageType * >(
        this->ProcessObject::GetInput(0)), 
        outputRegionForThread);
      it.GoToBegin();

      // fill vector with gradient iterators
      typedef ImageRegionConstIterator< GradientImageType > 
    ...
    ::ComputeReconstructionMatrix()
  {
    ...
    if( m_NumberOfGradientDirections < 6 )
    {
      itkExceptionMacro( << "Not enough gradient directions supplied." );
    }

    {
      ...
      {
        for(GradientDirectionContainerType::ConstIterator gdcit2 = 
          this->m_GradientDirectionContainer->Begin();
          gdcit2 != this->m_GradientDirectionContainer->End(); ++gdcit2)
        {
          if(gdcit1.Value() == gdcit2.Value() && gdcit1.Index() != 
            gdcit2.Index())
          {
            itkWarningMacro( << "Some of the Diffusion Gradients equal "
              << "each other. Corresponding image data should be averaged "
    ...
    if(!this->m_NumberOfEquatorSamplingPoints)
      ...
      = (int) ceil((double)sqrt(
      8*QBALL_RECON_PI*this->m_NumberOfGradientDirections));
    ...
    vnl_matrix_fixed<double, 3, NOdfDirections>* U =
      itk::PointShell<NOdfDirections, 
      vnl_matrix_fixed<double, 3, NOdfDirections> >::DistributePointShell();

    vnl_matrix_fixed<double, 3, NBasisFunctionCenters>* V =
      itk::PointShell<NBasisFunctionCenters, 
      vnl_matrix_fixed<double, 3, NBasisFunctionCenters> >
    ...
    vnl_matrix<double> *S
      = new vnl_matrix<double>(
        3,m_NumberOfEquatorSamplingPoints*NOdfDirections);
...
#ifdef __QBR_TIMING__
  m_Collector.Start( "ChoosingSigma" );
#endif //__QBR_TIMING__

    // determine interpolation kernel width first
    // use to determine diffusion measurement contribution 
    // to each of the kernels
    vnl_matrix<double> *H_plus
      = new vnl_matrix<double>(
        NBasisFunctionCenters,m_NumberOfGradientDirections);
...

    {
      ...
      vnl_matrix<double> *H 
        = new vnl_matrix<double>(
          m_NumberOfGradientDirections,NBasisFunctionCenters,(double)0);

      {
        ...
        for( double sigma=start; sigma<maxSigma; sigma+=stepsize)
        {
          vnl_matrix<double> *tmpH
            = new vnl_matrix<double>(
              m_NumberOfGradientDirections,NBasisFunctionCenters);

          for(unsigned int r=0; r<m_NumberOfGradientDirections; r++)
          {
            for(unsigned int c=0; c<NBasisFunctionCenters; c++)
            {
...
#ifdef __QBR_TIMING__
  m_Collector.Stop( "ChoosingSigma" );
#endif //__QBR_TIMING__

    std::cout << "Computing Reconstruction Matrix ... ";

#ifdef __QBR_TIMING__
  m_Collector.Start( "MatrixBuilding" );
#endif //__QBR_TIMING__
    ...
    vnl_matrix<double> *G
      = new vnl_matrix<double>(
        m_NumberOfEquatorSamplingPoints*NOdfDirections,NBasisFunctionCenters);

    {
      for(unsigned int r=0; 
          r<m_NumberOfEquatorSamplingPoints*NOdfDirections; 
          r++)
      {
        for(unsigned int c=0; c<NBasisFunctionCenters; c++)
        {
...
#ifdef __QBR_TIMING__
  m_Collector.Start( "GtimesHplus" );
#endif //__QBR_TIMING__

    vnl_matrix<double> *GH_plus = 
      new vnl_matrix<double>(
      m_NumberOfEquatorSamplingPoints*NOdfDirections,
      m_NumberOfGradientDirections);
    *GH_plus = (*G) * (*H_plus);

#ifdef __QBR_TIMING__
  m_Collector.Stop( "GtimesHplus" );
#endif //__QBR_TIMING__
    ...
    m_ReconstructionMatrix
      = new vnl_matrix<TOdfPixelType>(
        NOdfDirections,m_NumberOfGradientDirections,0.0);
    for(int i=0; i<NOdfDirections; i++)
    {
      for(unsigned int j=0; j<m_NumberOfGradientDirections; j++)
      {
        ...
        {
          (*m_ReconstructionMatrix)(i,j) += 
            (TOdfPixelType)(*GH_plus)(m_NumberOfEquatorSamplingPoints*i+k,j);
        }
      }
    }
...
#ifdef __QBR_TIMING__
  m_Collector.Stop( "MatrixBuilding" );
#endif //__QBR_TIMING__
    ...
    const GradientImageType *gradientImage )
  {
    ...
    if( m_GradientImageTypeEnumeration == GradientIsInASingleImage)
    {
      itkExceptionMacro( << "Cannot call both methods:" 
        << "AddGradientImage and SetGradientImage. "
        << "Please call only one of them.");
    }
    ...
    m_GradientDirectionContainer->InsertElement( 
      this->m_NumberOfGradientDirections, 
    ...
    const GradientImagesType *gradientImage )
  {
    ...
    for(GradientDirectionContainerType::Iterator it = 
      this->m_GradientDirectionContainer->Begin();
    ...
    this->m_NumberOfGradientDirections = 
      numImages - this->m_NumberOfBaselineImages;

    // ensure that the gradient image we received has as many components as 
    // the number of gradient directions
    if( gradientImage->GetVectorLength() != 
      this->m_NumberOfBaselineImages + m_NumberOfGradientDirections )
    {
      itkExceptionMacro( << m_NumberOfGradientDirections 
        << " gradients + " << this->m_NumberOfBaselineImages
        << "baselines = " 
        << m_NumberOfGradientDirections + this->m_NumberOfBaselineImages
        << " directions specified but image has " 
        << gradientImage->GetVectorLength()
        << " components.");
    }
    ...
    ::PrintSelf(std::ostream& os, Indent indent) const
  {
    ...
    os << indent << "OdfReconstructionMatrix: " 
      << m_ReconstructionMatrix << std::endl;
    ...
    {
      os << indent << 
        "GradientDirectionContainer: (Gradient directions not set)" 
        << std::endl;
    }
    ...
    os << indent << "Threshold for reference B0 image: " 
